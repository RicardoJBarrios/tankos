...
- Technical lifecycle timestamps are generated by the persistence boundary's
  injected `@tank-os/time` `ClockPort` and normalized to UTC. A trusted host
  normally supplies `TimeService`; standalone adapters may use their documented
  provider fallback. Writes return their projected record without a
  read-after-write.
